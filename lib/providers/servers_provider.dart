...
class ServersProvider with ChangeNotifier {
    ...
    try {
      if (_selectedServer != null) {
        await Prefs.setString(
            _selectedServerKey, json.encode(_selectedServer!.toJson()));
  ...
  Future<void> getServers() async {
    try {
      ...
      if (freeServers.isEmpty || proServers.isEmpty) {
        final free = await VpnServerHttp().getServers("free");
        setFreeServers(free);
        areServersLoading = false;

        final pro = await VpnServerHttp().getServers("premium");
        setProServers(pro);
      }
    } catch (e, stack) {
